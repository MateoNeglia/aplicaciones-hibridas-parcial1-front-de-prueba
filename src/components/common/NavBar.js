...
const NavBar = ({ onSearch }) => { 
  ...
  const handleSearch = ({ query, filters, sort }) => {
    console.log('NavBar: Handling search:', { query, filters, sort });
  ...
  return (
    <nav className="navbar">
      <div className="navbar-container">
        ...
        <button className="navbar-profile-btn" onClick={handleProfileClick}>
          <svg
            className="user-icon"
        ...
        { isAdmin ? <button className="navbar-admin-btn" onClick={() => navigate('/admin')}>Admin</button> : ''}
      </div>
    </nav>
  );
};
